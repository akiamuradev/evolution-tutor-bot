...
**Portfolio project demonstrating AI-assisted development, async Python backend, Telegram bot, aiohttp HTTP API, PostgreSQL, task-based RAG, React/Vite, VK Bridge and Docker.**

## Demo

Скриншоты и GIF продукта пока не закоммичены в публичный репозиторий. В репозитории есть только статические логотипы VK Mini App: `frontend/vk-mini-app/public/logo-dark-theme.png` и `frontend/vk-mini-app/public/logo-light-theme.png`.

Рекомендуемые пути для будущих демо-материалов:

```text
docs/screenshots/telegram-chat.png
docs/screenshots/vk-mini-app.png
...
- Not included in this public repo: product screenshots, GIF demo, production database dump and a dedicated unit/integration test suite.
...
Product screenshots and GIFs are not included in the public repository yet. Recommended future paths:
...
This is an AI-assisted portfolio project. AI tools helped speed up implementation, refactoring and documentation. The demonstrated engineering work is product decomposition, architecture decisions, integration of backend/API/frontend/database/LLM components, repository cleanup, factual documentation and GitHub-ready packaging.
